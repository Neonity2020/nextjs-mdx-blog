...
export default function Navigation() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-opacity-70 backdrop-blur-md bg-background/80 dark:bg-gray-800/80 shadow-sm">
      <div className="w-full relative flex items-center justify-between max-w-4xl mx-auto px-4 py-5">
        <Link href="/" className="font-bold text-3xl">
          Neonity&apos;s <span className="text-green-700">Blog</span> 
        </Link>
        <div className="hidden md:flex items-center space-x-6">
          <Link href="/" className="text-sm font-medium hover:text-green-500 font-bold">
            主页
          </Link>
          <Link href="/blog" className="text-sm font-medium hover:text-green-500 font-bold">
            博客
          </Link>
          <Link href="/about" className="text-sm font-medium hover:text-green-500 font-bold">
            关于
          </Link>
          <ModeToggle />
        </div>
